...
class ThreadGroup::Impl {
 ...
 private:
  void SetThreadAffinity(std::thread::native_handle_type thread, std::vector<unsigned int> ids) {
#if defined(__linux__) || defined(__ANDROID__)
...
#define CPU_ZERO(cpusetp) memset((cpusetp), 0, sizeof(cpu_set_t))
    ...
    if (sorted_order_.size() < static_cast<unsigned int>(num_workers_)) {
      if (mode == kSpecify) {
        ...
        for (unsigned i = 0; i < threads_.size(); ++i) {
          SetThreadFullCpuAffinity(threads_[i].native_handle(), false);
        }
      } else {
  ...
  void SetThreadFullCpuAffinity(std::thread::native_handle_type thread, bool reverse) {
  ...
  void SetMasterThreadFullCpuAffinity(bool reverse) {
    SetThreadFullCpuAffinity(pthread_self(), reverse);
  }
